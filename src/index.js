...
app.get(
    ...
    async function(req, res) {
        const client = global.client;
         ...
         .setTitle("Banned User Login")
         .setDescription(
             "<:add:946594917596164136> " +
             `[**${a.username}**#${a.discriminator}]` +
             " Tried to Login but has been banned from the site!"
         )
         .setColor("Red")
         .addFields({
             name: "Date",
     ...
     logs.send({
         ...
         req.session.destroy(() => {
         res.json({ error: true, login: false, message: "You have been blocked from Universe Lists.", logout: true })
         req.logout();
         return;
         });
         //-Ban Check End-\\
...
app.get("/user/ban", checkAuth, checkAdmin, async (req, res) => {
    ...
    return res.json({
        error: false,
        message: "user has been banned from the botlist"
    });
});

app.post("/user/unban", checkAuth, checkAdmin, async (req, res) => {
    ...
    banSchema.deleteOne({
        ...
        return res.json({ 
            error: true,
            message: "some error occurred check console"
        })
    })
    return res.json({
        error: false,
        message: "user has been unbanned from the botlist"
    });
});
//-Other Pages-//
