window.initiateMulti = function () {
  ...
  $(".multiselect").each(function (i, obj) {
    ...
    var innerHTML = "";
    var selectedItems = []; // Initialize an empty array for selected items
    $(`[data-link=${$(obj).attr("data-link")}] > option`).each(function (
      i,
      obj
    ) {
    ...
    function genHTMLInput() {
      ...
      selectedItems.map((item) => {
        ...
        const listItem = $(
          `${multiselectAttr} > .multiselect-list > [data-value=${item}]`
        );
    ...
    $(document).ready(function () {
      $(document).on("click", ".multiselect-close", function (e) {
        ...
        selectedItems.splice(
          selectedItems.indexOf(parent.attr("data-value")),
          1
        );
        const listItem = $(
          `${multiselectAttr} > .multiselect-list > [data-value=${parent.attr(
            "data-value"
          )}]`
        );
