...
app.get("/bots/:id/vote", checkAuth, async (req, res) => {
    ...
    if (!user) await global.userModel.create({
        id: req.user.id
    });
...
app.get("/api/bots/:id/voted", async (req, res) => {
    ...
    const vote = canUserVote(x);
    if (vote.status) return res.json({
        voted: false
    });
    return res.json({
        voted: true,
...
app.get("/servers/:id", async (req, res) => {
    ...
    if (server.published === false) {
        ...
        const member = await global.sclient.guilds.fetch(server.id).then(guild => guild.members.fetch(req.user.id));
        if (!member || (!member.permissions.has("ADMINISTRATOR") && !server.owner.includes(req.user.id)))
...
app.get("/servers/:id/edit", checkAuth, async (req, res) => {
    ...
    const member = await global.sclient.guilds.fetch(server.id).then(guild => guild.members.fetch(req.user.id));
    if (!member || (!member.permissions.has("ADMINISTRATOR") && req.user.id !== server.owner))
        return res.redirect("/403");
...
function canUserVote(x) {
    ...
    if (left <= 0 || formatted.includes("-")) return {
        status: true
    };
    return {
        status: false,
